...
const Bot = function(botPath) {
  ...
  //The IRC connection for the user
  this.userIRC = IRC(this.config['user'],
    this.config['user-oauth'], this.config['channels']);
  ...
  this.botIRC.on('message', data => {
    if(!this.chatters.has(data.username)) {
      //This will ensure the member is created when they send a message (if not already)
      this.db.members.single(data.username, error => {});
  ...
  this.botIRC.on('command', data => {
    ...
    this.event.emit('post-command', data);
  });

  //The user connect is only needed to capture the bot's messages
  this.userIRC.on('message', data => {
    if(data.username === this.config['bot-user']) {
      this.event.emit('message', data);
    }
  });
};
...
Bot.prototype.on = function(eventType, handler) {
  ...
  if(Bot.ircEvents.indexOf(eventType) >= 0) {
...
//Messaging Functions
Bot.prototype.send = function(message, channel, sendAsUser = false) {
  let msg = `#${(channel || this.config.channels[0])} :${message}`;

  if(sendAsUser) {
    this.userIRC.send('PRIVMSG', msg);
    return;
  }

  this.botIRC.send('PRIVMSG', msg);
};
...
Bot.ircEvents = ['channel', 'join', 'names', 'part', 'ban'];
