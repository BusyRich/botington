...
const Bot = function(botPath) {
  ...
  this.config['bot-username'] = this.config['bot-username'].toLowerCase();
  ...
  this.db.members.single = (username, callback) => {
    this.db.members.read({username:username}, 
      (error, members) => callback(error, (members ? members[0] : null)));
  };
